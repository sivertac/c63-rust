...
fn read_file_to_buf(file: &mut std::fs::File, target_buffer: &mut [u8], bytes_to_read: usize) -> Result<usize, io::Error> {
    let mut len: usize = 0;
...
pub fn read_yuv(file: &mut std::fs::File, ctx: &encode_context::EncodeContext) -> Result<c63::YUV, io::Error> {
    ...
    if len <= 0 {
        return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "File empty, read returned 0"));
    }
